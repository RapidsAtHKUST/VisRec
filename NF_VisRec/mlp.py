...
#   angles, radii = np.arctan2(y_coords, x_coords), np.sqrt(x_coords**2 + y_coords**2)

#   angles_tensor = torch.tensor(angles)
#   radii_tensor = torch.tensor(radii)

#   temp = torch.stack([angles_tensor, radii_tensor], dim=1)
#   x = temp.repeat(x.shape[0], 1, 1).to(x.device)
  rets = [x]
...
def fourierfeat_enc(x, B):
    ...
    # angles_tensor = torch.tensor(angles)/6.2831853
    # radii_tensor = torch.tensor(radii)

    # x = torch.stack([angles_tensor, radii_tensor], dim=1).repeat(x.shape[0], 1, 1).to(x.device)
