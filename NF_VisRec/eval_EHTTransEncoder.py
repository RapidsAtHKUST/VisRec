...
class NF_Model(pl.LightningModule):
    ...
    def __init__(
        ...
        encoder = m_encoder.Visibility_Graphormer(
    ...
    def forward(self, x, z):
        ...
        # print("pred_visibilities", pred_visibilities.shape)
    ...
    def inference_w_conjugate(self, uv_coords, z, nF=0, return_np=True):
        ...
        # print("reshape_pred_vis_imag", pred_vis_imag.shape)
        plt.imsave(fname=" "+str(ti)+"/recon_imag.png", arr=pred_vis_imag[0].cpu().detach().numpy(), cmap="viridis")
        plt.imsave(fname=" "+str(ti)+"/recon_real.png", arr=pred_vis_real[0].cpu().detach().numpy(), cmap="viridis")
        np.save(" "+str(ti)+"/recon_vis.npy", np.stack((pred_vis_real[0].cpu().detach().numpy(), pred_vis_imag[0].cpu().detach().numpy()), axis=0))
    ...
    def _step(self, batch, batch_idx, num_zero_samples=0):
        ...
        inputs_encoder = torch.cat([pe_uv, vis_sparse], dim=-1)
        z = self.context_encoder(inputs_encoder, uv_coords)
    ...
    def test_step(self, batch, batch_idx):
        ...
        if not self.use_unet:
            if self.loss_type=='spectral':
                ...
                # noise = torch.randn_like(vis_sparse)   # Random values from a Gaussian distribution with mean=0 and std_dev=0.05
                # vis_sparse = vis_sparse + 3 * noise  # Add the noise to the original data

                # # choose a random number between lower and upper
                # num_vectors_to_mask = int(vis_sparse.size(1) * 0.15)

                # # randomly select indices in the second dimension (1660 vectors)
                # masked_indices = torch.randperm(vis_sparse.size(1))[:num_vectors_to_mask]

                # # mask the selected indices by setting them to zero (or whatever value you want)
            ...
            plt.imsave(fname=" "+str(ti)+"/GT_imag.png", arr=vis_imag[0].cpu().detach().numpy(), cmap="viridis")
            plt.imsave(fname=" "+str(ti)+"/GT_real.png", arr=vis_real[0].cpu().detach().numpy(), cmap="viridis")
            np.save(" "+str(ti)+"/GT_vis.npy", np.stack((vis_real[0].cpu().detach().numpy(), vis_imag[0].cpu().detach().numpy()), axis=0))
        ...
        if self.norm_fact is None: # get the normalization factor, which is fixed given image/spectral domain dimensions
            ...
            _, _, norm_fact = make_dirtyim(uv_dense_physical,
                                   ...
                                   img_res, eht_fov, return_im=True)
            self.norm_fact= norm_fact
            # print("norm_fact",self.norm_fact)
            print('initiliazed the norm fact')
        ...
        if self.use_unet:
            ...
            # print("uv_dense.shape, z.shape", uv_dense.shape, z.shape)
            pred_fft = self.inference_w_conjugate(uv_dense, z, return_np=False)
            # print("pred_fft.shape", pred_fft.shape)
            uv_dense_physical = (uv_dense.detach().cpu().numpy()[0,:,:] +0.5)*(2*max_base) - (max_base)
            # print("uv_arr:",self.uv_arr.shape)
            # print("pred_fft:",pred_fft.shape)
        ...
        # img_diff = img_recon_gt.reshape(-1, img.shape[-1]).cpu() - img_recon.reshape(-1, img_recon.shape[-1]).cpu()
        
        # img_log = torch.cat([img_log, img_diff], dim = -1)
        # plt.imsave(f'{self.test_fldr}/img_recon_{batch_idx:05d}.png', img_log,cmap='hot')
        # print(vis_maps.shape)
        ...
        # plt.imsave(f'{self.test_fldr}/diff_{batch_idx:05d}.png', img_diff,cmap='hot')
...
def cli_main():
    ...
    parser.add_argument('--m_epochs', default=1, type=int, help= '# of max training epochs [1000]')
